#!/bin/sh
#
# ROCKNIX Ports launcher — INSTALL SD multiboot. This one writes.
#
...
#
# Run "1 Check" first. It is the same code path stopped before the first
# write, so a clean result there means this has already passed every
# safety gate.
#
# Afterwards: no card boots stock from internal NAND, a card with a
# bootable U-Boot boots that OS instead.
#
...
exec sh "$APP/install-multiboot.sh"
