...
|-----------|------|------------|-----|----------------|
| **Upper** (top) | Host | `usb_host0_ehci` `fd800000` **and** `usb_host0_ohci` `fd840000` | `usb2phy1_otg` | `phy-supply = <&vcc5v0_host>` (GPIO4_PC5 off `dcdc_boost`). Only connector that can power a bus-powered device. A high-inrush hub already inserted at power-on can brown out the board on battery. OHCI clocks: the three CRU clocks from `rk356x-base.dtsi` **plus** `<&usb2phy1>` (**480 MHz**; stock name `"utmi"`). |
| **Lower** (bottom) | Charge + gadget | `usb_host0_xhci` `fcc00000` | `usb2phy0_otg` | `dr_mode = "otg"`, `extcon = <&usb2phy0>`. **No** `phy-supply` — ID-grounding can flip the controller to host, but nothing raises VBUS. |
| **WiFi RTL8733BU** | Internal host | `usb_host1_ehci` `fd880000` | `usb2phy1_host` | Analog supply `vcc_3v3` (stock used 5 V). Enable GPIO is GPIO0_PA0 via `rtl8733bu_power`. |
| No connector | — | `usb_host1_xhci` `fd000000` | `usb2phy0_host` | USB2-only (`maximum-speed = "high-speed"`). Kept off the WiFi PHY. |
| Disabled | WiFi OHCI companion | `usb_host1_ohci` `fd8c0000` | — | That PHY carries only the high-speed RTL8733BU; the companion is never used. |
| Disabled | — | `combphy1` / `combphy2` | — | No USB3 / SATA / PCIe on this board. |

`otg_switch` on the RK817 is that PMIC's OTG boost. It is **not** the upper VBUS rail (`vcc5v0_host` is).

Earlier trees treated `usb2phy1_otg` / `usb_host0_ehci` as unused and disabled them to dodge a suspend hang; that is what first removed host ([06fd5cd0](https://github.com/Zetarancio/distribution/commit/06fd5cd044) put them back). A second mistake was leaving **`usb_host0_ohci` disabled**. EHCI hands full-speed devices to that companion (`PORT_OWNER`); with nowhere to land, a hub draws VBUS and never enumerates. High-speed sticks never take that path, which is why the first test passed. Stock enables this OHCI.

Enabling the companion **without its PHY clock** is not enough. `rk356x-base.dtsi` lists only three CRU clocks, so `usb480m` stayed at `enable_count` 0. OHCI needs the **48 MHz** derived from that 480 MHz; EHCI at high speed does not, which is why only this controller was affected. With the clock unclaimed and the controller live, every driver suspend callback returned, then the SoC hung in `suspend_ops->enter()` (firmware SMC, no console). The board DTS adds `<&usb2phy1>` as a fourth clock, matching stock `"utmi"`. Three suspend cycles pass with `vcc5v0_host` still `off-in-suspend` ([54d8b02425](https://github.com/Zetarancio/distribution/commit/54d8b02425); a clock-less enable was [reverted](https://github.com/Zetarancio/distribution/commit/35309e3fd1)).

The lower port staying host-capable for a bus-powered device is a **charger-driver** gap (stock raises `OTG_SWITCH` when ID is grounded; mainline `rk817_charger` does not), not a DTS one. `vcc5v0_host` is on-demand (`off-in-suspend`, not stock's always-on) so probe does not energise the rail before the PHY asks for it.
...
After each merge from `upstream/next`, re-check these **Miyoo Flip invariants** (none of the fork’s suspend/DMC/RK817 patches are in upstream `next` as of merge **`9c5dee6d29`**):
...
| Upper USB-C host: `usb2phy1_otg` + `usb_host0_ehci` + **`usb_host0_ohci`** with `clocks` including `<&usb2phy1>` + `phy-supply = <&vcc5v0_host>` | Do not re-disable as “unused”; OHCI is required for full-speed devices and needs the PHY **480 MHz** or suspend hangs in firmware |
