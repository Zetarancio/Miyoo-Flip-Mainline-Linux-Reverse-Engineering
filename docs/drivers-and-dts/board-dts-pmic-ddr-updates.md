...
|-----------|------|------------|-----|----------------|
| **Upper** (top) | Host | `usb_host0_ehci` `fd800000` **and** `usb_host0_ohci` `fd840000` | `usb2phy1_otg` | `phy-supply = <&vcc5v0_host>` (GPIO4_PC5 off `dcdc_boost`). Only connector that can power a bus-powered device. A high-inrush hub already inserted at power-on can brown out the board on battery. |
...
| **USB** | **Upper** (top) host: `usb2phy1_otg` + `usb_host0_ehci` + **`usb_host0_ohci`** + `vcc5v0_host`. **Lower** (bottom): charge/gadget, no VBUS. Do not re-disable the upper EHCI/PHY or its OHCI companion. [USB](#usb). |
...
| Upper USB-C host: `usb2phy1_otg` + `usb_host0_ehci` + **`usb_host0_ohci`** + `phy-supply = <&vcc5v0_host>` | Do not re-disable as “unused”; the OHCI companion is required for full-speed devices |
