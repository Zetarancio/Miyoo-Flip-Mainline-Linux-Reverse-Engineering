...
The app is **[`preloader-stock-rocknix/App/apommel-multiboot/`](https://github.com/Zetarancio/Miyoo-Flip-Mainline-Linux-Reverse-Engineering/tree/main/preloader-stock-rocknix/App/apommel-multiboot)**. It validates, backs up, writes, verifies, and rolls back if the readback does not match. Full option list and safety checks are in its own README.

Copy the **whole folder** to the card. On device, the practical way is to pick a script in a file manager and choose **Execute**:

| Script | Does |
|--------|------|
| **`install-multiboot.sh`** | patch the preloader → SD multiboot on |
| **`restore-preloader.sh`** | put a stock preloader back → multiboot off |

Each writes its own log beside the script (`install-log.txt`, `restore-log.txt`), which matters because a file manager usually shows no console. From a shell the same thing is `sh launch.sh [install|restore|backup]`.
...
Stock's partitions come from `mtdparts=` on the kernel command line and begin at `vnvm` (`0x200000`), so nothing maps `0x000000`–`0x200000`, and `CONFIG_MTD_PARTITIONED_MASTER` is not set. Erase can bypass the kernel through the SFC because **erase needs no ECC**; a write cannot, because NAND pages need correct ECC and only the MTD layer produces it.

Nor can stock be given a node. Its `mtdparts=` is not hardcoded — stock U-Boot derives it from the **GPT in flash**, whose first entry starts at LBA 4096, and the GPT's own entry array sits at `0x400`–`0x4400`: **the table you would have to edit is inside the region you cannot write.** ROCKNIX escapes this only because it takes its partitions from its device tree instead. Offsets and the reasoning are in the [app README](https://github.com/Zetarancio/Miyoo-Flip-Mainline-Linux-Reverse-Engineering/tree/main/preloader-stock-rocknix/App/apommel-multiboot#why-stock-cannot-simply-be-given-a-node).

**From a stock-only device:** run the [Preloader Eraser](stock-rocknix-without-disassembly.md#preloader-eraser--maskrom-access) → boot ROCKNIX from SD → run the app there. Two reboots, same end state.

### A backup is not automatically a restore point

If the preloader was **already erased** when a backup was taken — which is the case for anyone who used the eraser to get to ROCKNIX — that backup is **2 MiB of `0xff`** (md5 `b23b5d09162b92c0284923a7f628d2a5`). Writing it back erases rather than restores. The app rejects such images and skips them when auto-selecting, but verify your own:
...
Rather than rest on that, the app **verifies it per device**: it reads the existing preloader before erasing and compares the real ddrbin (`0x20620`–`0x2e000`, md5 `4824552a71c46199c7c260d68b6a831f`), refusing without erasing anything if it differs. Board model, SoC and NAND geometry are gated too.
...
### What Knulli / GammaOS would need to change

Ship a **Miyoo Flip** U-Boot FIT in the card's `uboot` partition (or at sector 16384), built so the stock SPL can load it — ROCKNIX's `flip` build is a working reference. Nothing in NAND needs to change, and no user-side flashing is involved; it is purely a change to what the image writes to the card.

Until then, users of those distros keep the [erase method](stock-rocknix-without-disassembly.md#preloader-eraser--maskrom-access), which boots the card's own idbloader from sector 64 — at the cost of internal stock boot.
...
| Topic | Link |
|-------|------|
| Eraser (MASKROM), restore script, recovery | [Stock ↔ ROCKNIX without disassembly](stock-rocknix-without-disassembly.md) |
