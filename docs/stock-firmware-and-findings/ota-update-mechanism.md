...
**A preloader is distributed, but over USB and not by Miyoo.** The `unbrick_tool_windows` package from [steward-fu's releases](https://github.com/steward-fu/website/releases?q=flip) — `update.img`, a Rockchip `RKFW` carrying `MiniLoaderAll` — contains one, flashed from MASKROM with RKDevTool. Its DRAM blob is the public Rockchip `v1.18` ddrbin — the same 54.5 KiB that every Flip preloader carries, distributed in one package for all units, not calibrated per device. That is what makes a shared patched preloader defensible; see [BSP and DDR findings](bsp-and-ddr-findings.md#1-ddr-initialization-and-firmware).
